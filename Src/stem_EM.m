...
classdef stem_EM < EM
    ...
    methods
        ...
        function st_EM_result = estimate(obj)
            ...
            if obj.stem_EM_options.max_iterations==0
                %In this case only the E-step is executed and only one
                %time. Iteration is set to -1 to enter the while cycle
                iteration=-1;
            else
                iteration=0;
            end
        ...
        function model_changed = M_step(obj,E_wb_y1,sum_Var_wb_y1,diag_Var_wb_y1,cov_wb_z_y1,E_wp_y1,sum_Var_wp_y1,diag_Var_wp_y1,cov_wp_z_y1,M_cov_wb_wp_y1,cov_wpk_wph_y1,diag_Var_e_y1,E_e_y1,sigma_eps,st_kalmansmoother_result,iteration)
            ...
            if par.stem_par_constraints.sigma_eps_fixed==0
                ...
                %st_par_em_step.sigma_eps(end,end)=par.sigma_eps(end,end);
                ct2=clock;
                ...
                if obj.stem_EM_options.wpar_estimation==1
                        ...
                        for z=1:K
                            ...
                            for k=kindex
                                ...
                                for h=hindex
                                    ...
                                    ctv1=clock;
                                    if not(obj.stem_model.stem_data.stem_modeltype.is('Emulator'))
                                        if size(par.v_p(:,:,z),1)==1
                                            initial=par.v_p(1,1,z);
                                            ...
                                            if (h~=k)
                                                initial=[par.v_p(k,k,z) par.v_p(h,h,z) par.v_p(k,h,z)];
                                                min_result = fminsearch(@(x) stem_EM.geo_coreg_function_velements(x,k,h,par.v_p(:,:,z),par.theta_p(z),par.correlation_type,obj.stem_model.DistMat_p,...
                                                    obj.stem_model.stem_data.stem_varset_p.dim,temp,T,obj.stem_model.stem_data.stem_gridlist_p.tap),initial,optimset('MaxIter',fminsearch_max_iter,'TolFun',0.01,'UseParallel','always'));

                                                ctv2=clock;
                                                disp(['    v_p(',num2str(h),',',num2str(h),'), v_p(',num2str(k),',',num2str(k),'), v_p(',num2str(h),',',num2str(k),') updating ended in ',stem_misc.decode_time(etime(ctv2,ctv1))]);
                                                v_temp(k,k,z)=min_result(1);
                                                v_temp(h,h,z)=min_result(2);
                                                v_temp(h,k,z)=min_result(3);
                                                v_temp(k,h,z)=min_result(3);
                                            end
                                        end
                                    else
                    ...
                    if not(obj.stem_model.stem_data.stem_modeltype.is('Emulator'))
                        for z=1:K
                            ...
                            %st_par_em_step.theta_p(z)=par.theta_p(z); %% TO REMOVE!
...
%                         end
    ...
    methods (Static)
        ...
        function f = geo_coreg_function_velements(v_element,row,col,v,theta,correlation_type,DistMat,var_dims,U,T,tapering_par)
            %DESCRIPTION: log-likelihood evaluation with respect to an extra-diagonal element of v_b or v_p
            %
            %INPUT
            %v_element          - [double]      (1x1) the extra-diagonal element of v_b or v_p
            ...
            n_var=length(var_dims);
            v(row,row)=v_element(1);
            v(col,col)=v_element(2);
            v(col,row)=v_element(3);
            v(row,col)=v_element(3);
